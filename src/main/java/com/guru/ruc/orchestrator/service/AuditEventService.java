...
import com.guru.ruc.orchestrator.config.audit.AuditEventConverter;
import com.guru.ruc.orchestrator.repository.PersistenceAuditEventRepository;
...
public class AuditEventService {
	...
	public Optional<AuditEvent> find(Long id) {
		return Optional.ofNullable(persistenceAuditEventRepository.findById(id)).filter(Optional::isPresent)
				.map(Optional::get).map(auditEventConverter::convertToAuditEvent);
	}
}
